...
public class FloatWindowService extends Service {
	...
	private class RefreshTask extends TimerTask {
		...
		public void run() {
			// 当前界面没有悬浮窗显示，则创建悬浮
			if (!FloatWindowManager.getInstance(getApplicationContext()).isWindowShowing()) {
				handler.post(new Runnable() {
					@Override
					public void run() {
						FloatWindowManager.getInstance(getApplicationContext())
								.createSmallWindow(getApplicationContext(), layoutResId,
										rootLayoutId);
					}
				});
			}
